...
def _build_ledger_structured_block(
    ...
    parts = [
        ...
        "EVERY FIGURE HERE WAS ADDED UP BY THE PROGRAM from the cells of the "
        "Excel file, not written by a model and not read off an image. Totals "
        "and rankings are arithmetic over the sheet named in "
        "\"source_sheet_name\". Where the sheet also printed its own total row, "
        "the two were compared and any disagreement is reported in "
        "\"extraction_notes\" — read those notes before relying on an account. "
        "When a figure decides a credit conclusion, still say which file "
        "(\"source_files\"), which sheet and which account it came from, so the "
